...
/**
 * 多路复用服务端单线程V3，用到写事件
 * 考虑到可能有些读写事件比较耗时，所以单独将他们放到另外的线程执行（充分利用cpu核数）（由于主线程可能会多次执行该读写事件，所以需要先执行key.cancel()）
 * 缺点：执行cancel是走系统调用
 * 后续优化：在N个读写事件FD就绪时，将这些FD分组，每组分一个selector，每组里面的读写事件是串行执行的，此时不需要上面的再开辟线程单独处理从而引发cancel系统调用。
 *          从而引出多路复用器多线程版本！
 */
public class SocketMultiplexingSingleThreadV3 {
                ...
                while (selector.select(500) > 0) {
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (key.isAcceptable()) {
                            ...
                            key.cancel(); //【注意】这里另起了线程会有时间差，所以当前循环又会走到这并执行可读事件,所以需要提前把这个key从多路复用器中拿出去，所以底层这里是一个【系统调用】
    ...
    public void readHandler(SelectionKey key) {
        new Thread(() -> {
            ...
            try {
                ...
                System.out.println("...read handler结束...");
            } catch (IOException e) {
                e.printStackTrace();
            }
        }).start();
    ...
    public static void main(String[] args) {
        SocketMultiplexingSingleThreadV3 service = new SocketMultiplexingSingleThreadV3();
        service.start();
    }
}
